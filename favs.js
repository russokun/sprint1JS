...
document.addEventListener('moviesLoaded', () => {
  const favsIds = new Set(JSON.parse(localStorage.getItem('favoritos')) || []);
  const favMovies = movies.filter(movie => favsIds.has(movie.id));
  let Card = (title, image, tagline, overview, id, isChecked) => {
    // Limitar la descripción (overview) a 50 caracteres
    const overviewLimitado = overview.length > 50 ? overview.slice(0, 50) + '...' : overview
    
    let cardHTML = `
        <div class="relative max-w-xs bg-[#ADADAD] shadow-lg rounded-lg overflow-hidden mr-4 mb-4 data-id="${id}">
        <label class="container">
          <input type="checkbox" id="${id}" ${isChecked ? 'checked' : ''}> <!-- Añadir el atributo checked si isChecked es true -->
  ...
  function CardsMovies(moviesArray) {
    ...
    moviesArray.forEach(function(movie){
      // Verificar si la película está en favsIds
      const isChecked = favsIds.has(movie.id);
      // Llamar a la función crearCard() con los datos de cada movie y concatenar el HTML resultante
      cardsHTML += Card(movie.title, movie.image, movie.tagline, movie.overview, movie.id, isChecked);
    })
    return cardsHTML
  }
  const contenedor = document.getElementById("favs");
        contenedor.innerHTML = CardsMovies(favMovies);
